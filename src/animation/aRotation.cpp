...
void quat::FromRotation(const mat3& rot)
{
    ...
    if (rot[0][0] + rot[1][1] + rot[2][2] > 0) {
        double denom = sqrt(rot[0][0] + rot[1][1] + rot[2][2] + 1.0) * 2;
        mQ[VW] = denom / 4;
        mQ[VX] = (rot[2][1] - rot[1][2]) / denom;
        mQ[VY] = (rot[0][2] - rot[2][0]) / denom;
        mQ[VZ] = (rot[1][0] - rot[0][1]) / denom;
    }
    else if ((rot[0][0] > rot[1][1]) && (rot[0][0] > rot[2][2])) {
        double denom = sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]) * 2;
        mQ[VX] = denom / 4;
        mQ[VW] = (rot[2][1] - rot[1][2]) / denom;
        mQ[VZ] = (rot[0][2] + rot[2][0]) / denom;
        mQ[VY] = (rot[1][0] + rot[0][1]) / denom;
    }
    else if ((rot[1][1] > rot[2][2])) {
        double denom = sqrt(1 - rot[0][0] + rot[1][1] - rot[2][2]) * 2;
        mQ[VY] = denom / 4;
        mQ[VZ] = (rot[2][1] + rot[1][2]) / denom;
        mQ[VW] = (rot[0][2] - rot[2][0]) / denom;
        mQ[VX] = (rot[1][0] + rot[0][1]) / denom;
    }
    else {
        double denom = sqrt(1 - rot[0][0] - rot[1][1] + rot[2][2]) * 2;
        mQ[VZ] = denom / 4;
        mQ[VY] = (rot[2][1] + rot[1][2]) / denom;
        mQ[VX] = (rot[0][2] + rot[2][0]) / denom;
        mQ[VW] = (rot[1][0] - rot[0][1]) / denom;
    }
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    double cosDot = acos(dot);
    double sinCosDot = sqrt(1.f - pow(dot, 2));
    if (fabs(sinCosDot) < 0.001) {
        q = 0.5 * q0 + 0.5 * q2;
        return q.Normalize();
    }
    q = (sin((1 - u) * cosDot) * q0 / sinCosDot) + (q2 * sin(cosDot * u) / sinCosDot);
    return q.Normalize();
}
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
    ...
    if (angleRad != 0) {
        axis = vec3(mQ[VX], mQ[VY], mQ[VZ]) / sin(angleRad / 2);
    }
}
