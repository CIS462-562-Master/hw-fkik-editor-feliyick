...
void AJoint::updateTransform()
{
	...
	AJoint* parent = this->getParent();
	if (parent == nullptr) {
		this->setLocal2Global(this->getLocal2Parent());
	}
	else {
		this->setLocal2Global(parent->getLocal2Global() * this->getLocal2Parent());
	}
	for each (AJoint* j in mChildren) {
		j->updateTransform();
	}
